<template>
  <v-container class="status-history-container">
    ...
    <v-data-table :headers="headers" :items="statusHistory" item-value="id" class="elevation-1" :items-per-page="10"
      :footer-props="footerProps">
      <template v-slot:footer="{ props }">
        <div class="v-data-table-footer__items-per-page">
          <span>{{ itemsPerPageText }}</span>
          <v-select v-model="props.itemsPerPage" :items="props.rowsPerPageItems" class="ml-2" dense
            hide-details></v-select>
        </div>
        <div class="v-data-table-footer__pagination">
          <span>{{ paginationText(props.pageStart, props.pageStop, props.itemsLength) }}</span>
          <v-pagination v-model="props.page" :length="props.pages" class="ml-2" total-visible="7"></v-pagination>
        </div>
      </template>
...
import fetchStatusHistory from '@/services/statusHistoryService.js';
...
.status-history-container {
  ...
  /* Match the drawer background color */
  padding: 20px;
  border-radius: 10px;
